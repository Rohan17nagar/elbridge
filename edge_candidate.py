...
class Candidate():
    ...
    def crossover(self, other):
        ...
        chromosomes = [(self.chromosome[i], other.chromosome[i]) if random.random() < 0.5 else
                       (other.chromosome[i], self.chromosome[i]) for i in
                       range(len(self.chromosome))]
        chromosome_a = list(map(itemgetter(0), chromosomes))
        chromosome_b = list(map(itemgetter(1), chromosomes))
